...
def menu(con, header, options, width, screen_width, screen_height):
    if len(options) > 20:
        raise ValueError('Cannot have a menu with more than 20 options.')
...
def main_menu(con, background_image, screen_width, screen_height):
    ...
    libtcod.console_print_ex(0, int(screen_width / 2), int(screen_height / 2) - 10, libtcod.BKGND_NONE, libtcod.CENTER, 'ATRL: A Tiny RogueLike')
    libtcod.console_print_ex(0, int(screen_width / 2), int(screen_height - 2), libtcod.BKGND_NONE, libtcod.CENTER, 'by alphaconverter')
